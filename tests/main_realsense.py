...
PCD_PATH = "./data/Tetris_views/"
CAD_PATH = "./data/CAD_models"
TARGET_PTS    = 1000
TRACK_EVERY = 1
        ...
        while True:
            ...
            if not initialized:
                ...
                while frame_counter!=10:
                    ...
                    # Check wheter a mask was detected, if not continue with next frame
                    if len(detections) == 0:
                        continue
                ...
                H_init, _ = estimator.find_best_template_teaser(dst_cloud, class_id)
            ...
            start = time.time()
            prev_down, mesh = estimator.create_template_from_H(T_m2c=T_m2c, target_points=TARGET_PTS, class_id=class_id)
